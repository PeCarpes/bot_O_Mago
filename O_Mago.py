...
async def on_member_join(member: discord.Member):
    ...
    channel = discord.utils.get(guild.text_channels, name=server_config["canal_introducao"])
    if not channel:
        print(f"ERRO: Canal '{server_config['canal_introducao']}' não encontrado no servidor {guild.name}.")
        return

    # ... (o resto do código do embed é igual) ...
    embed = discord.Embed(title=f"Bem-vindo(a) à aventura, {member.name}!", description=f"Eu sou O Mago, o guardião deste servidor de RPG. Para começarmos, por favor, responda a esta mensagem enviando **apenas o nome do seu personagem**.\n\n*Você tem 5 minutos para responder.*", color=discord.Color.purple())
    ...
    try:
        response_message = await bot.wait_for('message', timeout=300.0, check=check)
        player_name = response_message.content
        await response_message.delete()
    except asyncio.TimeoutError:
        await channel.send(f"{member.mention}, parece que você se perdeu na floresta... \n Você demorou demais para responder. Entre em contato com o Mestre")
        return
    ...
    if role:
        ...
        print(f"ERRO: O cargo '{server_config['cargo_jogador']}' não foi encontrado.")
        return
    
    # Usa a config do servidor para encontrar a categoria
    category = discord.utils.get(guild.categories, name=server_config["categoria_privados"])
    if not category:
        category = await guild.create_category(server_config["categoria_privados"])

    overwrites = { guild.default_role: discord.PermissionOverwrite(read_messages=False), member: discord.PermissionOverwrite(read_messages=True, send_messages=True), guild.me: discord.PermissionOverwrite(read_messages=True) }
    private_channel = await guild.create_text_channel(name=f'diario-{player_name.lower().replace(" ", "-")}', overwrites=overwrites, category=category, topic=f"Diário de Bordo de {player_name}.")
    
    await channel.send(f"Perfeito, {member.mention}! Seu cargo foi concedido e o canal {private_channel.mention} foi criado. Boa sorte!")
    await private_channel.send(f"Olá, {player_name}! Este é o seu canal privado.")
...
async def ping(ctx):
    ...
    await ctx.send(f"🏓 Pong! A minha velocidade de resposta é de `{latency_ms}ms`.")
...
async def roll(ctx, *, formula: str):
    ...
    try:
        ...
        for part in parts:
            if 'd' in part:
                ...
                if not (0 < num_dice <= 100 and 0 < num_sides <= 1000):
                    await ctx.send("Valores de dados inválidos. Use entre 1-100 dados e 1-1000 lados.")
                    return
        ...
        await ctx.send(f"Não entendi a fórmula `{formula}`. Por favor, use o formato `!roll XdY + Z`.")
...
async def set_canal_intro(ctx, *, nome_canal: str):
    ...
    await ctx.send(f"✅ O canal de introdução deste servidor foi definido para `{nome_canal}`.")
...
async def set_cargo_jogador(ctx, *, nome_cargo: str):
    ...
    await ctx.send(f"✅ O cargo de jogador deste servidor foi definido para `{nome_cargo}`.")
...
async def set_categoria_privada(ctx, *, nome_categoria: str):
    ...
    save_configs() # Salva a alteração
    await ctx.send(f"✅ A categoria de canais privados deste servidor foi definida para `{nome_categoria}`.")
...
# --- Executando o Bot ---
BOT_TOKEN = 'MTM3OTkwMDY4MDQ2MTg3NzM3OQ.GHUgEU.ZojLYPediR5UDNcOpoawPkJRrdODqaDu935-LI'
